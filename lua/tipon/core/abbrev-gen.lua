...
local json_path = vim.fn.stdpath("config") .. "/abolish_data.json"

-- Load and parse JSON once on plugin init
...
-- Build flat lookup table: full_abbrev -> expansion
-- This runs once; handles all root + suffix combos explicitly from JSON
M.abbrevs = {}
local data = load_json_data()
for _, entry in ipairs(data) do
	...
	for i, s_abbrev in ipairs(suffix_abbrevs) do
		local full_abbrev = root_abbrev .. s_abbrev
		local s_word = suffix_words[i] or "" -- Fallback to empty if mismatch
		M.abbrevs[full_abbrev] = root_word .. s_word
	end
end
